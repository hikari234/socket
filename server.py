            ...
            try:
                ...
                if command in ['UPLOAD', 'DOWNLOAD', 'RETRIEVE']:
                    if len(msg_pieces) < 2:
                        ...
                        self.request.send(b'ERROR: please provide file name')
                        time.sleep(SENDING_COOLDOWN)
                    elif len(msg_pieces) > 2:
                        print(timestamp(), 'ERROR: to many arguments')
                        self.request.send(b'ERROR: to many arguments')
                        time.sleep(SENDING_COOLDOWN)
                    elif command == 'UPLOAD':
                        self.filename = msg_pieces[1]
                        if self.filename in self.saved_files:
                            print(timestamp(), 'ERROR: the file is there')
                            self.request.send(b'ERROR: the file is there')
                            time.sleep(SENDING_COOLDOWN)
                        else:
                        ...
                        if self.filename not in self.saved_files:
                            ...
                            self.request.send(
                                b'ERROR: the file does not exist')
                            time.sleep(SENDING_COOLDOWN)
                        else:
                            self.request.send(b'SUCCESS')
                            time.sleep(SENDING_COOLDOWN)
                            ...
                            for line in content:
                                ...
                                print(timestamp(), 'Receive ACK for line: {}'.format(
                                    line.rstrip()))
                            self.request.send(b'#')
                    ...
                    self.request.send(b'TCP connection closed')
                    time.sleep(SENDING_COOLDOWN)
                else:
                    print(timestamp(), 'ERROR: unknown command')
                    self.request.send(b'ERROR: unknown command')
                    time.sleep(SENDING_COOLDOWN)
            except Exception:
                pass
